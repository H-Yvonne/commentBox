...
(function (root, factory) {
    if (typeof exports === 'object') {
        var $;
        try { $ = require('../lib/jquery'); } catch(e) {};
        module.exports = factory($);
    } else if (typeof define === 'function' && define.amd) {
        define(['../lib/jquery'], function ($) {
            return (root.uploader = factory($));
        });
    } else {
    ...
    var staticID = 0;
    // var upCnt_tpl = '<div class="uploaderCnt" style="width:0px;height:0px;position:fixed;right:0px;top:0px;overflow:hidden"></div>';
    var up_tpl = ['<div class="uploaderItem uploader{ID}" style="position:absolute;right:0px;top:0px;overflow:hidden;opacity:0;filter:alpha(opacity=0);">',
    ...
    function iframe_load(iframe,callback){
        if (iframe.attachEvent){
            iframe.attachEvent("onload", function(){
                callback&&callback.call(iframe.Document);
            });
        }else{
    ...
    function SingleUp(param){
        ...
        var wrap = param.btn.parents(param.wrap);
